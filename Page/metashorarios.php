...
<script>
                ...
                success: function(data) {
                    ...
                    data.forEach(function(employee) {
                        ...
                        var row = `<tr>
                            ...
                            <td contenteditable="true" class="percentage" data-original-percentage="${(100 / data.length).toFixed(2)}">${(100 / data.length).toFixed(2)}%</td>
                            <td class="meta"> 
                            Q ${formatNumberWithCommas (parseFloat(employee.META || 0).toFixed(2))} 
                            </td>
                            <td>
    ...
    $('#empleadosTable').on('click', '.save-meta', function() {
        ...
        function adjustMetasByPercentage(editedRow, newPercentage) {
            ...
            // Adjust if there's a rounding error
            ...
            // Now adjust the meta amounts based on new percentages
        ...
        function updateTotalMetas() {
            ...
            // Collect all metas and sum the total
            $('#empleadosTable tbody tr').each(function() {
                ...
                if (!isNaN(metaValue)) {
                    ...
                    metaValues.push(0); // Ensure there is a value for each row, even if zero
                }
            });
            ...
            // Update each individual percentage and sum the percentages
            $('#empleadosTable tbody tr').each(function(index) {
                var percentage = (metaValues[index] / totalMetas) * 100;
                $(this).find('.meta-percentage').text(`${percentage.toFixed(2)}%`);
                totalPercentage += percentage;
            });

            // Display the total percentage at the bottom of the table
